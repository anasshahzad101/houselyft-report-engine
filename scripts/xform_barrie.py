"""
xform_barrie.py — adapt the House Lyft master report for
David David, 17 B Cundles Road East, Barrie, ON (interior basement apartment).

Scope: Basement Apartment -> 1 interior ARU, SCOPED render mode.
City has no zoning adapter -> rules researched live from official sources
(City of Barrie Zoning By-law 2009-141, Barrie ARU / Second Suites pages,
Ontario Bill 23, County of Simcoe). verified = False -> report-needs-review.

Programs cleared for a 1-unit interior suite in Barrie / Simcoe County:
  - Bill 23 / Barrie DC By-law 2023-074 ARU development-charge exemption
  - City of Barrie 2026 ARU permit-fee incentive
  - County of Simcoe Secondary Suites Program (figure hedged)
GST/HST PBRH (4+ units) -> moved into Option B as upside.
MLI Select (5+), ACLP ($1M), Toronto DC waiver, MHRTC -> dropped (unreachable
or unconfirmable), per docs/PROGRAM_GATING_v1.md.

Follows the scripts/xform_*.py pattern: every replacement must match exactly
once; a leftover grep guards against source-city data surviving.
"""
import os

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SRC = os.path.join(ROOT, "templates", "report_houselyft_master.html")
OUT = os.path.join(ROOT, "templates", "report_barrie.html")

s = open(SRC).read()
R = []

# ---- cover -----------------------------------------------------------------
R.append(('<div class="addr">303 Coxwell Avenue<span>Toronto, ON</span></div>',
          '<div class="addr">17 B Cundles Road East<span>Barrie, ON</span></div>'))

# ---- property barhead ------------------------------------------------------
R.append(('<div class="barhead">303 Coxwell Avenue, Toronto, ON&nbsp;&nbsp;M4L 3B5</div>',
          '<div class="barhead">17 B Cundles Road East, Barrie, ON&nbsp;&nbsp;L4M 5L1</div>'))

# ---- aerial placeholders -> honest pending line (no licensed source) -------
R.append(('''  <div class="imgrow" style="margin-top:0;">
    <div class="imgbox tall"><span class="ic">◎</span>Aerial view<br><small>(auto-generated)</small></div>
    <div class="imgbox tall"><span class="ic">▤</span>Street view<br><small>(auto-generated)</small></div>
  </div>
  <div class="imglicense" style="font-size:6pt;color:#9aa2b2;margin:-6px 0 8px;">Imagery: source and licence inserted at generation.</div>''',
'''  <div style="font-size:8.5pt;color:#7a818f;margin:2px 0 12px;font-style:italic;">Aerial and street-level photography pending a licensed imagery source.</div>'''))

# ---- property table 1 ------------------------------------------------------
R.append(('''    <tr><td>Property Address</td><td>303 Coxwell Avenue, Toronto, ON&nbsp;&nbsp;M4L 3B5</td></tr>
    <tr><td>Name</td><td>John Arockiaraj</td></tr>
    <tr><td>Phone Number</td><td>(647) 223-4342</td></tr>
    <tr><td>Email</td><td>johneeraj@gmail.com</td></tr>
    <tr><td>Development Goals</td><td>4+1 or 6+1 Multiplex; Maximize unit count; To Be Decided</td></tr>''',
'''    <tr><td>Property Address</td><td>17 B Cundles Road East, Barrie, ON&nbsp;&nbsp;L4M 5L1</td></tr>
    <tr><td>Name</td><td>David David</td></tr>
    <tr><td>Phone Number</td><td>Provided at intake</td></tr>
    <tr><td>Email</td><td>Provided at intake</td></tr>
    <tr><td>Development Goals</td><td>Basement apartment (interior secondary suite) for rental income; intends to keep the property</td></tr>'''))

# ---- property table 2 ------------------------------------------------------
R.append(('''    <tr><td>Municipality</td><td>Toronto</td></tr>
    <tr><td>Neighbourhood</td><td>Woodbine Corridor / Upper Beaches</td></tr>
    <tr><td>Ward</td><td>Ward 19 — Beaches-East York</td></tr>
    <tr><td>Community League</td><td>Greenwood-Coxwell / Upper Beaches</td></tr>
    <tr><td>Waste Collection</td><td>Contact City of Toronto for local schedule</td></tr>
    <tr><td>Current Bylaw</td><td>Toronto Zoning By-law 569-2013</td></tr>
    <tr><td>Legal Description</td><td>To be confirmed</td></tr>
    <tr><td>Year Built</td><td>Pre-1980 (Att/Row/Townhouse, 1.5 storey)</td></tr>
    <tr><td>Lot size</td><td>~315.9 m² (20 ft × 170 ft / approx. 3,400 sq ft)</td></tr>
    <tr><td>Development Goals</td><td>6+1 Multiplex (primary); 4+1 Multiplex (alternative)</td></tr>''',
'''    <tr><td>Municipality</td><td>City of Barrie</td></tr>
    <tr><td>Neighbourhood</td><td>Cundles East</td></tr>
    <tr><td>County (Service Manager)</td><td>County of Simcoe</td></tr>
    <tr><td>Waste Collection</td><td>City of Barrie curbside schedule</td></tr>
    <tr><td>Current Bylaw</td><td>City of Barrie Comprehensive Zoning By-law 2009-141</td></tr>
    <tr><td>Legal Description</td><td>To be confirmed</td></tr>
    <tr><td>Year Built</td><td>To be confirmed</td></tr>
    <tr><td>Lot size</td><td>To be confirmed</td></tr>
    <tr><td>Development Goals</td><td>Basement apartment (interior secondary suite); optional additional ARU(s)</td></tr>'''))

# ---- neighbourhood spotlight ----------------------------------------------
R.append(('''    <div class="ct">Neighbourhood Spotlight</div>
    303 Coxwell Avenue is located in the Woodbine Corridor / Upper Beaches neighbourhood at the intersection of Coxwell Ave and Gerrard Street East — one of Toronto's most walkable and transit-connected east-end communities:
    <ul>
      <li>Borders Greenwood-Coxwell, Danforth, and The Beaches neighbourhoods</li>
      <li>Rocca's No Frills grocery steps away; Coxwell subway station approximately 1 km north</li>
      <li>Multiple TTC routes at the intersection: Coxwell (22), Carlton (506), and more</li>
      <li>Regular bus service along 87, 92, and 95 Avenues and 156, 163, and 170 Streets</li>
      <li>Greenwood Park approximately 9-minute walk; restaurants and retail along Gerrard Street East and Danforth Avenue</li>
    </ul>''',
'''    <div class="ct">Neighbourhood Spotlight</div>
    17 B Cundles Road East is in the Cundles East neighbourhood in Barrie's northeast — an established residential area with steady rental demand:
    <ul>
      <li>Just west of Highway 400, with quick access across the city and toward the GTA</li>
      <li>Approximately 1.2 km from Georgian College — a consistent source of rental demand</li>
      <li>Served by Barrie Transit along the Cundles Road East corridor</li>
      <li>An active development corridor, with several medium-density projects underway nearby</li>
    </ul>'''))

# ---- zoning table ----------------------------------------------------------
R.append(('''    <tr><td>Current Zoning</td><td>RD — Residential Detached (Toronto Zoning By-law 569-2013)</td></tr>
    <tr><td>Minimum Site Requirements</td><td>Official Plan Designation: Neighbourhoods. Ward 19 (Beaches-East York) is in the Toronto &amp; East York Community Council district — one of nine wards where 6-unit houseplexes are permitted as-of-right under By-law 654-2025.</td></tr>
    <tr><td>Recent Changes</td><td>Up to 4 units as-of-right city-wide (By-law 474-2023); up to 6 units as-of-right in Ward 19 — Toronto &amp; East York district (By-law 654-2025, June 2025). No rezoning required for either scenario.</td></tr>
    <tr><td>Permitted Uses</td><td>Multi-Unit Housing Types — the RD zone in Ward 19 (Toronto &amp; East York district) allows up to <strong>6 residential units</strong> as-of-right in a detached houseplex under By-law 654-2025, subject to technical review of site conditions.</td></tr>
    <tr><td>Development Goals Achievable?</td><td><strong>YES</strong>; proceed to Step 2 — <strong>Builder Ready Package™</strong></td></tr>''',
'''    <tr><td>Current Zoning</td><td>Residential — City of Barrie Comprehensive Zoning By-law 2009-141 (exact residential zone confirmed in Phase 2)</td></tr>
    <tr><td>Minimum Site Requirements</td><td>A serviced residential lot (municipal water &amp; sewer) within Barrie's settlement area — the basis for as-of-right additional residential units.</td></tr>
    <tr><td>Recent Changes</td><td>Ontario's Bill 23 (More Homes Built Faster Act) requires up to 3 residential units as-of-right on serviced residential lots. The City of Barrie goes further, permitting <strong>up to 4 residential units per lot</strong> — the primary dwelling plus up to three additional residential units. No rezoning required.</td></tr>
    <tr><td>Permitted Uses</td><td>An interior secondary suite (such as a basement apartment) is permitted as-of-right, subject to Barrie's site standards and the Second Suites Registration By-law 2020-010. Confirmed in Phase 2.</td></tr>
    <tr><td>Development Goals Achievable?</td><td><strong>YES</strong>; proceed to Step 2 — <strong>Builder Ready Package™</strong></td></tr>'''))

# ---- "what this means for you" list ---------------------------------------
R.append(('''      <li><strong>Townhouse &amp; Stacked Townhouse:</strong> Multi-unit attached homes — side-by-side units (townhouse) or vertically stacked units (stacked townhouse)</li>
      <li><strong>Detached Houseplex / Semi-detached Houseplex:</strong> Standalone multi-unit homes</li>
      <li><strong>Low-Rise Apartment Building:</strong> Small-scale apartment buildings</li>
      <li><strong>Internal &amp; Backyard Suites:</strong> Secondary suites (like basement or garden suites) can be seamlessly paired with main dwellings to boost density</li>''',
'''      <li><strong>Interior Basement Apartment:</strong> a self-contained unit within your existing home — your primary goal</li>
      <li><strong>Additional Residential Units:</strong> Barrie permits up to four units per lot, so a basement suite can be paired with further ARUs where the site allows</li>
      <li><strong>Detached / Garden Suite:</strong> a self-contained suite in the rear yard is also an ARU option under Barrie's rules</li>
      <li><strong>Registered &amp; Legal:</strong> second suites are registered with the City and inspected for fire and property-standards safety</li>'''))

# ---- time-sensitive --------------------------------------------------------
R.append(('''    <div class="d"><div class="dt">Ontario HST Rebate — Act Now<br><small>~ 6 weeks from now</small></div><div class="dx">Ontario's 2026 Budget (March 26, 2026) introduced a 100% rebate of the 8% provincial HST component on new purpose-built rental housing, stacking on top of the existing federal PBRH rebate. For units valued up to $1M, savings can reach $80,000 per unit in provincial relief alone. This is a temporary enhancement — applications require the agreement be signed between April 1, 2026 and March 31, 2027. Structuring the project correctly from Day 1 is essential to capture this.</div></div>
    <div class="d"><div class="dt">DC Waiver — Already in Effect<br><small>~ 3 weeks from now</small></div><div class="dx">Development charges are fully eliminated for multiplexes up to 6 units in Toronto (Bill 185, January 2025). This saves $200,000–$270,000 per project — approximately $45,000–$50,000 per unit — at no application required. This benefit is locked in as long as your project stays within the 6-unit as-of-right envelope.</div></div>
    <div class="d"><div class="dt">CMHC</div><div class="dx">Canada Mortgage Housing Corporation (CMHC) policy changes can occur at any time, potentially affecting financing options. It is recommended to submit your application as early as possible to reduce any risk.</div></div>''',
'''    <div class="d"><div class="dt">Barrie 2026 ARU Permit-Fee Incentive — Act in 2026<br><small>this calendar year</small></div><div class="dx">For 2026, the City of Barrie is offering 50% off the building-permit application fee for additional residential units, with a full rebate of the fee if the unit reaches final inspection/occupancy within 12 months. Applications run January 1 to December 31, 2026, with projects complete by December 31, 2027. Starting this year captures the incentive.</div></div>
    <div class="d"><div class="dt">County of Simcoe Secondary Suites — Budget-Limited<br><small>first-come, first-served</small></div><div class="dx">The County of Simcoe offers a forgivable loan to help create an affordable second suite, available to City of Barrie residents. Funding is budget-limited and allocated first-come, first-served; the current amount and intake status are confirmed in Phase 2. Applying early improves your chances.</div></div>
    <div class="d"><div class="dt">CMHC</div><div class="dx">Canada Mortgage Housing Corporation (CMHC) policy changes can occur at any time, potentially affecting financing options. It is recommended to submit your application as early as possible to reduce any risk.</div></div>'''))

# ---- rezoning: co-green ----------------------------------------------------
R.append(('<div class="co-green"><div class="ct2">Not Required for This Property</div>The recommended 6+1 configuration is permitted as-of-right under Toronto By-law 654-2025.</div>',
          '<div class="co-green"><div class="ct2">Not Required for This Property</div>An interior basement apartment is permitted as-of-right under the City of Barrie\'s Zoning By-law 2009-141 and Ontario\'s Bill 23 — no rezoning needed.</div>'))

# ---- rezoning: comparison table row ---------------------------------------
R.append(('<tr><td>What governs your build</td><td class="g">By-law 654-2025</td><td class="n">A new site-specific by-law</td></tr>',
          '<tr><td>What governs your build</td><td class="g">By-law 2009-141</td><td class="n">A new site-specific by-law</td></tr>'))

# ---- rezoning: two cards ---------------------------------------------------
R.append(('''    <div class="card2"><div class="ct">Six-unit houseplex</div>Ward 19 sits in the Toronto &amp; East York district, one of nine wards where By-law 654-2025 permits up to six units in a residential zone without rezoning.</div>
    <div class="card2"><div class="ct">Rear garden suite</div>Toronto's Garden Suite By-law (February 2022) permits a rear ancillary suite on a non-laneway lot as-of-right in residential zones.</div>''',
'''    <div class="card2"><div class="ct">Additional residential units</div>Barrie permits up to four residential units per lot — the primary dwelling plus up to three ARUs — without rezoning, subject to site standards.</div>
    <div class="card2"><div class="ct">Detached / garden suite</div>A self-contained suite in the rear yard is also permitted as an ARU under Barrie's rules, as an alternative or addition to an interior suite.</div>'''))

# ---- rezoning: "what this means for ..." ----------------------------------
R.append(('''  <div class="barhead" style="text-align:left;">What this means for 303 Coxwell Avenue</div>
  <p>Because 303 Coxwell Avenue already permits the recommended build under existing zoning, no rezoning application is contemplated in this analysis. Your project advances directly to design and permitting. The comparison below shows what that avoids. This assessment reflects the by-laws in force at the date of this report and is subject to technical review of site conditions.</p>''',
'''  <div class="barhead" style="text-align:left;">What this means for 17 B Cundles Road East</div>
  <p>Because a basement apartment is permitted as-of-right on this lot under existing zoning, no rezoning application is contemplated in this analysis. Your project advances directly to permitting and registration. The comparison above shows what that avoids. This assessment reflects the by-laws in force at the date of this report and is subject to technical review of site conditions.</p>'''))

# ---- rezoning: co-amber confirm item --------------------------------------
R.append(('<div class="co-amber"><b>One item to confirm: the permit status of the existing rear garage.</b><br><span class="sub">If it was converted without a permit, a retroactive application is needed before financing or development can proceed.</span></div>',
          '<div class="co-amber"><b>One item to confirm: the basement\'s Building Code compliance.</b><br><span class="sub">A legal basement apartment must meet the Ontario Building Code (ceiling height, egress, fire separation) and be registered with the City under By-law 2020-010. Any existing unpermitted work should be confirmed before financing or development proceeds.</span></div>'))

# ---- options: A ------------------------------------------------------------
R.append(('<div class="opt"><div class="oh">Option A — 4-Unit Multiplex + 1 Garden Suite (4+1)</div>',
          '<div class="opt"><div class="oh">Option A — Interior Basement Apartment (your goal)</div>'))
R.append(('''      <div class="od">A detached 4-unit houseplex on the main structure, plus one garden suite in the rear utilizing the existing ~750 sq ft converted garage. Total: 5 independent units. Fully as-of-right under By-law 474-2023. No rezoning, no variances likely required if designed within the standard envelope. The existing garage — with its 12 ft ceilings, heated floors, running water, and powder room — provides a strong head start on the ancillary suite. Lot: 20 ft x 170 ft (~315.9 m²). Corner lot configuration with street frontage on Coxwell Ave and a flanking side street. RD zone built-form standards apply. No parking minimums for multiplexes. Development charges fully waived for ≤6 units (Bill 185).</div>''',
'''      <div class="od">A self-contained basement apartment within your existing home, rented for ongoing income while you keep the property — your stated goal. Permitted as-of-right under Barrie's Zoning By-law 2009-141 and Ontario's Bill 23; no rezoning. The unit must meet the Ontario Building Code (ceiling height, egress, fire separation), carry a building permit, and be registered with the City under the Second Suites Registration By-law 2020-010. One parking space is required for the unit. Development charges are exempt for the first two additional residential units on the property.</div>'''))

# ---- options: B ------------------------------------------------------------
R.append(('<div class="opt"><div class="oh">Option B — 6-Unit Multiplex + 1 Garden Suite (6+1) — Primary Recommendation</div>',
          '<div class="opt"><div class="oh">Option B — Basement Apartment + Additional ARU — Up to 4 Units (upside)</div>'))
R.append(('''      <div class="od">A detached 6-unit houseplex (as-of-right in Ward 19 under By-law 654-2025) plus one garden suite in the rear. Total: 7 independent units. This matches John's stated goal. The 20 ft frontage on a 170 ft deep lot is narrow but workable for a stacked or back-to-back configuration. The corner lot provides an additional entrance point improving unit separation and access. A minor variance may be required depending on the final design footprint. No parking spaces required. Development charges fully waived.</div>''',
'''      <div class="od">Barrie permits up to four residential units per lot, so the basement apartment can be paired with one or more additional residential units — for example a detached rear suite — where the site and servicing allow. This maximizes rental income while keeping the property in your hands. If the project is built out to four or more self-contained rental units, the federal GST/HST Purpose-Built Rental Housing rebate may open up — it requires four or more self-contained rental units held 90%+ as long-term rental — explored in Phase 2. Unit sizes and siting are confirmed in Phase 2.</div>'''))

# ---- options: C ------------------------------------------------------------
R.append(('<div class="opt"><div class="oh">Option C — Note on the Existing Garage / Rear Suite</div>',
          '<div class="opt"><div class="oh">Option C — Registration, Parking &amp; Practical Steps</div>'))
R.append(('''      <div class="od">The existing ~750 sq ft garage (12 ft ceilings, heated floors, running water, powder room) may have been converted or is in process. Under Toronto's Garden Suite By-law (February 2022), a rear ancillary suite on a non-laneway lot is permitted as-of-right in residential zones. Confirming the permit status of this structure is an essential first step — both for financing qualification and for counting it as a legal unit. If the conversion was done without a permit, a retroactive permit application will be required before any development or financing process can proceed.</div>''',
'''      <div class="od">A legal second suite in Barrie is registered with the City under the Second Suites Registration By-law 2020-010, which triggers Property Standards and Fire Department inspections for Building-Code and fire safety. A building permit is required, and one parking space must be provided for the unit with a minimum 1.2 m-wide access path to it. Confirming the existing home's basement conditions — ceiling height, window egress, and separation — early is the key first step, both for the permit and for financing. These details are finalized in Phase 2.</div>'''))

# ---- goal summary ----------------------------------------------------------
R.append(('''  <div class="barhead" style="text-align:left;">6+1 Configuration</div>
  <p>303 Coxwell Avenue is in Ward 19 — one of only nine wards in Toronto where up to six units are permitted as-of-right in a residential zone under By-law 654-2025. <strong>The 6+1 configuration is the clear primary recommendation.</strong></p>''',
'''  <div class="barhead" style="text-align:left;">Interior Basement Apartment</div>
  <p>17 B Cundles Road East is a serviced residential lot in Barrie where an interior basement apartment is permitted as-of-right under Zoning By-law 2009-141 and Ontario's Bill 23 — matching your goal of adding rental income while keeping the property. <strong>The basement apartment is the clear primary recommendation</strong>, with additional residential units available as upside where the site allows.</p>'''))

# ---- summary: current zoning review ---------------------------------------
R.append(('''  <p>303 Coxwell Avenue confirms a strong development option. This property is located in Ward 19 (Beaches-East York) — one of only nine wards across Toronto where up to <strong>six units are permitted as-of-right in a residential zone</strong>. This regulatory advantage, secured through By-law 654-2025 (June 2025), is not available to most Toronto homeowners evaluating the same path.</p>
  <ul>
    <li><strong>The Six-Unit As-of-Right Advantage:</strong> This lot is in one of nine wards in Toronto where By-law 654-2025 allows a 6-unit houseplex as-of-right — no rezoning, no public hearing, no Council approval required.</li>
  </ul>''',
'''  <p>17 B Cundles Road East is a serviced residential lot in the City of Barrie. An interior basement apartment — your goal — is permitted as-of-right under Zoning By-law 2009-141 and Ontario's Bill 23, with no rezoning required, subject to the City's site standards and second-suite registration.</p>
  <ul>
    <li><strong>The As-of-Right Advantage:</strong> Barrie permits up to four residential units per lot, so a basement apartment adds a rental income stream using space you already own — with development charges exempt on the first two additional units and, for 2026, reduced building-permit fees.</li>
  </ul>'''))

# ---- grants injection (replace the GATED_GRANTS_ROWS marker block) --------
R.append(('''    <!-- GATED_GRANTS_ROWS
         Injected by the builder from config/programs.json AFTER apply_gates().
         EVERY grant/incentive is gated - none may be hardcoded here.
         Previously hardcoded (the defect this fixes): GST/HST PBRH (4+ units),
         CMHC ACLP ($1M+ loan), CMHC Prefab Plus (inherits MLI), Toronto DC Waiver
         (Toronto only - was appearing on Mississauga reports).
         scoped mode -> only programs clearing the stated scope.
         tiered mode -> each program attached to the smallest tier that clears it.
         See docs/PROGRAM_GATING_v1.md -->
    </table>''',
'''    <tr><td>Provincial</td><td>Development-Charge Exemption for Additional Residential Units (Bill 23)</td><td>Under Ontario's More Homes Built Faster Act (Bill 23) and the City of Barrie's Development Charges By-law 2023-074, the first two additional residential units on an existing residential property are exempt from development charges — a per-unit saving on a basement suite. Confirmed for your project in Phase 2. <small>(Ontario Bill 23; City of Barrie ARU page.)</small></td></tr>
    <tr><td>Municipal</td><td>City of Barrie — 2026 Additional Residential Unit Incentive</td><td>For 2026, the City of Barrie is offering 50% off the building-permit application fee for additional residential units, fully rebated where the unit reaches final inspection/occupancy within 12 months. Applications run January 1–December 31, 2026; projects complete by December 31, 2027. <small>(City of Barrie — 2026 ARU Incentives Program.)</small></td></tr>
    <tr><td>Municipal (County)</td><td>County of Simcoe — Secondary Suites Program</td><td>The County of Simcoe (the housing service manager for the area, including the City of Barrie) offers a multi-year forgivable loan to help create an affordable second suite, forgiven over the affordability term where program conditions are met. It is budget-limited and first-come, first-served; the current funding amount and intake status are confirmed in Phase 2. <small>(County of Simcoe — Affordable Housing Programs.)</small></td></tr>
    </table>'''))

# ---------------------------------------------------------------- apply
fails = 0
for old, new in R:
    c = s.count(old)
    if c != 1:
        print(f"[FAIL x{c}] {old[:70]!r}")
        fails += 1
    else:
        s = s.replace(old, new)

open(OUT, "w").write(s)

# ---------------------------------------------------------------- leftover check
print("--- leftover check ---")
for t in ["Coxwell", "Toronto", "Ward 19", "Beaches", "John Arockiaraj",
          "654-2025", "569-2013", "6+1", "4+1", "Bill 185", "houseplex",
          "garden suite in the rear", "MLI Select", "johneeraj"]:
    n = s.count(t)
    if n:
        print(f"LEFTOVER '{t}': {n}")
print(f"done. fails={fails}  ->  {OUT}")
