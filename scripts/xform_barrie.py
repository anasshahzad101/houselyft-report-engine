"""
xform_barrie.py — turn the House Lyft master report into the 103 Essa Road, Barrie lead report.

Same pattern as scripts/xform_cambridge.py: read the master, apply exact-match
replacements (each must hit exactly once), write templates/report_barrie.html,
then grep for leftovers from the source property/city.

Barrie has no zoning-engine adapter, so this report is a REVIEW-tier build:
  - Province-wide + City-of-Barrie facts only (Bill 23 floor; Barrie's 4-unit
    as-of-right per its 2024 Zoning By-law update; ARU DC exemption up to 2 units;
    Barrie 2026 ARU permit-fee incentive). No invented zone code, bylaw number,
    or program dollar amount — everything site-specific is hedged to Phase 2.
  - House Lyft prose sections kept verbatim; TM marks kept verbatim.
"""
import os

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SRC = os.path.join(ROOT, "templates", "report_houselyft_master.html")
OUT = os.path.join(ROOT, "templates", "report_barrie.html")

s = open(SRC).read()
R = []

# cover
R.append(('<div class="addr">303 Coxwell Avenue<span>Toronto, ON</span></div>',
          '<div class="addr">103 Essa Road<span>Barrie, ON</span></div>'))

# barhead (property details)
R.append(('<div class="barhead">303 Coxwell Avenue, Toronto, ON&nbsp;&nbsp;M4L 3B5</div>',
          '<div class="barhead">103 Essa Road, Barrie, ON&nbsp;&nbsp;L4N 3K8</div>'))

# imagery licence placeholder -> honest credit
R.append(('<div class="imglicense" style="font-size:6pt;color:#9aa2b2;margin:-6px 0 8px;">Imagery: source and licence inserted at generation.</div>',
          '<div class="imglicense" style="font-size:6pt;color:#9aa2b2;margin:-6px 0 8px;">Imagery: property aerial and street-level views are compiled during the feasibility phase (Phase 2).</div>'))

# property table 1
R.append(('''    <tr><td>Property Address</td><td>303 Coxwell Avenue, Toronto, ON&nbsp;&nbsp;M4L 3B5</td></tr>
    <tr><td>Name</td><td>John Arockiaraj</td></tr>
    <tr><td>Phone Number</td><td>(647) 223-4342</td></tr>
    <tr><td>Email</td><td>johneeraj@gmail.com</td></tr>
    <tr><td>Development Goals</td><td>4+1 or 6+1 Multiplex; Maximize unit count; To Be Decided</td></tr>''',
'''    <tr><td>Property Address</td><td>103 Essa Road, Barrie, ON&nbsp;&nbsp;L4N 3K8</td></tr>
    <tr><td>Name</td><td>Jason H</td></tr>
    <tr><td>Phone Number</td><td>Provided at intake</td></tr>
    <tr><td>Email</td><td>Provided at intake</td></tr>
    <tr><td>Development Goals</td><td>Multiplex development; maximize as-of-right units</td></tr>'''))

# property table 2
R.append(('''    <tr><td>Municipality</td><td>Toronto</td></tr>
    <tr><td>Neighbourhood</td><td>Woodbine Corridor / Upper Beaches</td></tr>
    <tr><td>Ward</td><td>Ward 19 — Beaches-East York</td></tr>
    <tr><td>Community League</td><td>Greenwood-Coxwell / Upper Beaches</td></tr>
    <tr><td>Waste Collection</td><td>Contact City of Toronto for local schedule</td></tr>
    <tr><td>Current Bylaw</td><td>Toronto Zoning By-law 569-2013</td></tr>
    <tr><td>Legal Description</td><td>To be confirmed</td></tr>
    <tr><td>Year Built</td><td>Pre-1980 (Att/Row/Townhouse, 1.5 storey)</td></tr>
    <tr><td>Lot size</td><td>~315.9 m² (20 ft × 170 ft / approx. 3,400 sq ft)</td></tr>
    <tr><td>Development Goals</td><td>6+1 Multiplex (primary); 4+1 Multiplex (alternative)</td></tr>''',
'''    <tr><td>Municipality</td><td>City of Barrie (single-tier)</td></tr>
    <tr><td>Neighbourhood</td><td>Allandale — south Barrie</td></tr>
    <tr><td>Transit</td><td>Essa Road corridor; near Allandale Waterfront GO (Barrie line) &amp; Barrie Allandale Transit Terminal</td></tr>
    <tr><td>Waste Collection</td><td>City of Barrie curbside schedule</td></tr>
    <tr><td>Current Bylaw</td><td>City of Barrie Comprehensive Zoning By-law — exact zone confirmed in Phase 2</td></tr>
    <tr><td>Legal Description</td><td>To be confirmed</td></tr>
    <tr><td>Year Built</td><td>To be confirmed</td></tr>
    <tr><td>Lot size</td><td>To be confirmed</td></tr>
    <tr><td>Development Goals</td><td>Multiplex — up to 4 units as-of-right (primary)</td></tr>'''))

# neighbourhood spotlight
R.append(('''    <div class="ct">Neighbourhood Spotlight</div>
    303 Coxwell Avenue is located in the Woodbine Corridor / Upper Beaches neighbourhood at the intersection of Coxwell Ave and Gerrard Street East — one of Toronto's most walkable and transit-connected east-end communities:
    <ul>
      <li>Borders Greenwood-Coxwell, Danforth, and The Beaches neighbourhoods</li>
      <li>Rocca's No Frills grocery steps away; Coxwell subway station approximately 1 km north</li>
      <li>Multiple TTC routes at the intersection: Coxwell (22), Carlton (506), and more</li>
      <li>Regular bus service along 87, 92, and 95 Avenues and 156, 163, and 170 Streets</li>
      <li>Greenwood Park approximately 9-minute walk; restaurants and retail along Gerrard Street East and Danforth Avenue</li>
    </ul>''',
'''    <div class="ct">Neighbourhood Spotlight</div>
    103 Essa Road is in Barrie's Allandale area in the city's south end, along the Essa Road corridor — an established, transit-connected part of Barrie:
    <ul>
      <li>On Essa Road, one of south Barrie's main arterial routes toward the downtown and waterfront</li>
      <li>Close to the Allandale Waterfront GO Station on GO Transit's Barrie line — direct rail service toward the GTA</li>
      <li>Near the Barrie Allandale Transit Terminal (Essa Road &amp; Gowan Street), a regional transit hub opened in 2025</li>
      <li>Walkable access to south-end shops and services and Lake Simcoe's Allandale waterfront and parks</li>
      <li>Steady rental demand from commuters and area employers, including Georgian College (illustrative context, not a valuation)</li>
    </ul>'''))

# zoning table
R.append(('''    <tr><td>Current Zoning</td><td>RD — Residential Detached (Toronto Zoning By-law 569-2013)</td></tr>
    <tr><td>Minimum Site Requirements</td><td>Official Plan Designation: Neighbourhoods. Ward 19 (Beaches-East York) is in the Toronto &amp; East York Community Council district — one of nine wards where 6-unit houseplexes are permitted as-of-right under By-law 654-2025.</td></tr>
    <tr><td>Recent Changes</td><td>Up to 4 units as-of-right city-wide (By-law 474-2023); up to 6 units as-of-right in Ward 19 — Toronto &amp; East York district (By-law 654-2025, June 2025). No rezoning required for either scenario.</td></tr>
    <tr><td>Permitted Uses</td><td>Multi-Unit Housing Types — the RD zone in Ward 19 (Toronto &amp; East York district) allows up to <strong>6 residential units</strong> as-of-right in a detached houseplex under By-law 654-2025, subject to technical review of site conditions.</td></tr>
    <tr><td>Development Goals Achievable?</td><td><strong>YES</strong>; proceed to Step 2 — <strong>Builder Ready Package™</strong></td></tr>''',
'''    <tr><td>Current Zoning</td><td>Residential (City of Barrie Comprehensive Zoning By-law) — exact zone confirmed in Phase 2</td></tr>
    <tr><td>Minimum Site Requirements</td><td>A serviced residential lot (municipal water &amp; sewer) within Barrie's settlement area — the basis on which additional residential units are permitted as-of-right. Site-specific standards confirmed in Phase 2.</td></tr>
    <tr><td>Recent Changes</td><td>Under Ontario's Bill 23 up to 3 units are permitted as-of-right province-wide; the City of Barrie updated its Zoning By-law (2024) to permit up to <strong>4 residential units</strong> on an eligible residential lot — no rezoning required. Confirm the governing zone standards in Phase 2.</td></tr>
    <tr><td>Permitted Uses</td><td>Multi-unit / additional residential units — on single-detached, semi-detached, and townhouse/rowhouse lots, Barrie permits up to <strong>4 units</strong> as-of-right (interior units and/or a detached additional unit), subject to the City's standards for height, setbacks, and size. Confirmed in Phase 2.</td></tr>
    <tr><td>Development Goals Achievable?</td><td><strong>YES</strong>; proceed to Step 2 — <strong>Builder Ready Package™</strong></td></tr>'''))

# what this means (housing types)
R.append(('''      <li><strong>Townhouse &amp; Stacked Townhouse:</strong> Multi-unit attached homes — side-by-side units (townhouse) or vertically stacked units (stacked townhouse)</li>
      <li><strong>Detached Houseplex / Semi-detached Houseplex:</strong> Standalone multi-unit homes</li>
      <li><strong>Low-Rise Apartment Building:</strong> Small-scale apartment buildings</li>
      <li><strong>Internal &amp; Backyard Suites:</strong> Secondary suites (like basement or garden suites) can be seamlessly paired with main dwellings to boost density</li>''',
'''      <li><strong>Detached / Semi-detached with additional units:</strong> up to four units created within and/or alongside the main dwelling</li>
      <li><strong>Townhouse / Rowhouse with additional units:</strong> eligible attached forms can also add additional residential units</li>
      <li><strong>Interior Suites:</strong> a second, third, or fourth unit created inside the existing home (for example, a basement or upper-floor suite)</li>
      <li><strong>Detached Additional Unit:</strong> a self-contained unit in a detached accessory building, subject to the City's height and setback standards</li>'''))

# time-sensitive block
R.append(('''    <div class="d"><div class="dt">Ontario HST Rebate — Act Now<br><small>~ 6 weeks from now</small></div><div class="dx">Ontario's 2026 Budget (March 26, 2026) introduced a 100% rebate of the 8% provincial HST component on new purpose-built rental housing, stacking on top of the existing federal PBRH rebate. For units valued up to $1M, savings can reach $80,000 per unit in provincial relief alone. This is a temporary enhancement — applications require the agreement be signed between April 1, 2026 and March 31, 2027. Structuring the project correctly from Day 1 is essential to capture this.</div></div>
    <div class="d"><div class="dt">DC Waiver — Already in Effect<br><small>~ 3 weeks from now</small></div><div class="dx">Development charges are fully eliminated for multiplexes up to 6 units in Toronto (Bill 185, January 2025). This saves $200,000–$270,000 per project — approximately $45,000–$50,000 per unit — at no application required. This benefit is locked in as long as your project stays within the 6-unit as-of-right envelope.</div></div>
    <div class="d"><div class="dt">CMHC</div><div class="dx">Canada Mortgage Housing Corporation (CMHC) policy changes can occur at any time, potentially affecting financing options. It is recommended to submit your application as early as possible to reduce any risk.</div></div>''',
'''    <div class="d"><div class="dt">Barrie ARU Incentive — 2026 Window<br><small>time-limited</small></div><div class="dx">The City of Barrie's 2026 Additional Residential Unit Incentives Program reduces building-permit application fees for new additional residential units, with a full rebate of those fees where occupancy is reached within 12 months of permit issuance. The program runs on a limited annual budget and applications close at year-end — current availability and amounts are confirmed in Phase 2.</div></div>
    <div class="d"><div class="dt">ARU Development Charges<br><small>provincial + municipal</small></div><div class="dx">Under provincial legislation, additional residential units are exempt from development charges — in Barrie, up to two additional units per property. A further unit may attract development charges. The exact treatment for your project is confirmed in Phase 2.</div></div>
    <div class="d"><div class="dt">CMHC</div><div class="dx">Canada Mortgage Housing Corporation (CMHC) policy changes can occur at any time, potentially affecting financing options. It is recommended to submit your application as early as possible to reduce any risk.</div></div>'''))

# rezoning co-green
R.append(('<div class="co-green"><div class="ct2">Not Required for This Property</div>The recommended 6+1 configuration is permitted as-of-right under Toronto By-law 654-2025.</div>',
          '<div class="co-green"><div class="ct2">Not Required for This Property</div>The recommended multiplex of up to four units is permitted as-of-right under the City of Barrie Zoning By-law — no rezoning required.</div>'))

# rezoning comparison row
R.append(('<tr><td>What governs your build</td><td class="g">By-law 654-2025</td><td class="n">A new site-specific by-law</td></tr>',
          '<tr><td>What governs your build</td><td class="g">City of Barrie Zoning By-law</td><td class="n">A new site-specific by-law</td></tr>'))

# rezoning twocard
R.append(('''    <div class="card2"><div class="ct">Six-unit houseplex</div>Ward 19 sits in the Toronto &amp; East York district, one of nine wards where By-law 654-2025 permits up to six units in a residential zone without rezoning.</div>
    <div class="card2"><div class="ct">Rear garden suite</div>Toronto's Garden Suite By-law (February 2022) permits a rear ancillary suite on a non-laneway lot as-of-right in residential zones.</div>''',
'''    <div class="card2"><div class="ct">Up to four units</div>The City of Barrie permits up to four residential units on an eligible residential lot as-of-right — no rezoning and no public hearing required.</div>
    <div class="card2"><div class="ct">Detached additional unit</div>An additional residential unit in a detached accessory building is permitted, subject to Barrie's height and setback standards — confirmed in Phase 2.</div>'''))

# rezoning barhead + para
R.append(('<div class="barhead" style="text-align:left;">What this means for 303 Coxwell Avenue</div>',
          '<div class="barhead" style="text-align:left;">What this means for 103 Essa Road</div>'))
R.append(('<p>Because 303 Coxwell Avenue already permits the recommended build under existing zoning, no rezoning application is contemplated in this analysis. Your project advances directly to design and permitting. The comparison below shows what that avoids. This assessment reflects the by-laws in force at the date of this report and is subject to technical review of site conditions.</p>',
          '<p>Because 103 Essa Road already permits the recommended build under existing zoning, no rezoning application is contemplated in this analysis. Your project advances directly to design and permitting. The comparison below shows what that avoids. This assessment reflects the by-laws in force at the date of this report and is subject to technical review of site conditions.</p>'))

# rezoning co-amber
R.append(('<div class="co-amber"><b>One item to confirm: the permit status of the existing rear garage.</b><br><span class="sub">If it was converted without a permit, a retroactive application is needed before financing or development can proceed.</span></div>',
          '<div class="co-amber"><b>One item to confirm: the governing zone and site standards for this lot.</b><br><span class="sub">Barrie\'s exact zone, lot dimensions, and servicing are confirmed in Phase 2 before design begins.</span></div>'))

# options A header + body
R.append(('<div class="opt"><div class="oh">Option A — 4-Unit Multiplex + 1 Garden Suite (4+1)</div>',
          '<div class="opt"><div class="oh">Option A — Multiplex, Up to Four Units (Primary Recommendation)</div>'))
R.append(('''      <div class="od">A detached 4-unit houseplex on the main structure, plus one garden suite in the rear utilizing the existing ~750 sq ft converted garage. Total: 5 independent units. Fully as-of-right under By-law 474-2023. No rezoning, no variances likely required if designed within the standard envelope. The existing garage — with its 12 ft ceilings, heated floors, running water, and powder room — provides a strong head start on the ancillary suite. Lot: 20 ft x 170 ft (~315.9 m²). Corner lot configuration with street frontage on Coxwell Ave and a flanking side street. RD zone built-form standards apply. No parking minimums for multiplexes. Development charges fully waived for ≤6 units (Bill 185).</div>''',
'''      <div class="od">A multiplex of up to four residential units on this lot, permitted as-of-right under the City of Barrie Zoning By-law — no rezoning required. Units can be created within and/or attached to the main dwelling. This directly matches the goal of a multiplex development and captures the maximum as-of-right unit count. Built-form standards — height, setbacks, coverage, and any parking — are set by Barrie's zoning by-law and confirmed against the lot in Phase 2. Additional residential units are exempt from development charges for up to two of the added units.</div>'''))

# options B header + body
R.append(('<div class="opt"><div class="oh">Option B — 6-Unit Multiplex + 1 Garden Suite (6+1) — Primary Recommendation</div>',
          '<div class="opt"><div class="oh">Option B — Four Units with a Detached Additional Unit</div>'))
R.append(('''      <div class="od">A detached 6-unit houseplex (as-of-right in Ward 19 under By-law 654-2025) plus one garden suite in the rear. Total: 7 independent units. This matches John's stated goal. The 20 ft frontage on a 170 ft deep lot is narrow but workable for a stacked or back-to-back configuration. The corner lot provides an additional entrance point improving unit separation and access. A minor variance may be required depending on the final design footprint. No parking spaces required. Development charges fully waived.</div>''',
'''      <div class="od">Up to four units arranged as interior or attached units in the main dwelling plus one self-contained unit in a detached accessory building at the rear. This suits lots with usable rear-yard depth and can improve unit separation and private access. The detached unit is subject to Barrie's height and setback standards for additional residential units, confirmed in Phase 2. This matches Jason's multiplex goal while adding a detached rental unit.</div>'''))

# options C header + body
R.append(('<div class="opt"><div class="oh">Option C — Note on the Existing Garage / Rear Suite</div>',
          '<div class="opt"><div class="oh">Option C — Phased Build / Rental Income</div>'))
R.append(('''      <div class="od">The existing ~750 sq ft garage (12 ft ceilings, heated floors, running water, powder room) may have been converted or is in process. Under Toronto's Garden Suite By-law (February 2022), a rear ancillary suite on a non-laneway lot is permitted as-of-right in residential zones. Confirming the permit status of this structure is an essential first step — both for financing qualification and for counting it as a legal unit. If the conversion was done without a permit, a retroactive permit application will be required before any development or financing process can proceed.</div>''',
'''      <div class="od">If a full four-unit build is not the immediate goal, the property can add one or two additional residential units first — for example an interior suite — then expand toward the four-unit envelope later. Additional residential units are development-charge exempt for up to two units, and Barrie's 2026 permit-fee incentive may reduce upfront costs (availability confirmed in Phase 2). This keeps carrying costs low while establishing rental income.</div>'''))

# goal summary
R.append(('''  <div class="barhead" style="text-align:left;">6+1 Configuration</div>
  <p>303 Coxwell Avenue is in Ward 19 — one of only nine wards in Toronto where up to six units are permitted as-of-right in a residential zone under By-law 654-2025. <strong>The 6+1 configuration is the clear primary recommendation.</strong></p>''',
'''  <div class="barhead" style="text-align:left;">Up to Four Units As-of-Right</div>
  <p>103 Essa Road is a serviced residential lot in the City of Barrie, where up to four residential units are permitted as-of-right — no rezoning required. <strong>A multiplex of up to four units is the clear primary recommendation</strong>, matching your stated multiplex goal, with a detached additional unit as an option where the rear yard allows.</p>'''))

# grants table — replace ONLY the Toronto municipal row (federal rows are national, kept verbatim)
R.append(('<tr><td>Municipal</td><td>Toronto Development Charge Waiver (Bill 185)</td><td>Development charges fully eliminated for multiplexes up to 6 units (Bill 185, January 2025). Saves $200,000–$270,000 per project (~$45,000–$50,000 per unit). Parking minimums also waived city-wide since February 2022, saving an additional $50,000–$100,000 in site costs. No application required — benefit applies automatically to compliant builds.</td></tr>',
          '<tr><td>Provincial / Municipal</td><td>ARU Development-Charge Exemption + Barrie ARU Incentive</td><td>Additional residential units are exempt from development charges under provincial legislation — in Barrie, up to two additional units per property. The City of Barrie\'s 2026 Additional Residential Unit Incentives Program also reduces building-permit application fees, with a full rebate where occupancy is reached within 12 months. Program budget is limited and amounts change — current availability confirmed in Phase 2.</td></tr>'))

# summary current zoning review
R.append(('''  <p>303 Coxwell Avenue confirms a strong development option. This property is located in Ward 19 (Beaches-East York) — one of only nine wards across Toronto where up to <strong>six units are permitted as-of-right in a residential zone</strong>. This regulatory advantage, secured through By-law 654-2025 (June 2025), is not available to most Toronto homeowners evaluating the same path.</p>
  <ul>
    <li><strong>The Six-Unit As-of-Right Advantage:</strong> This lot is in one of nine wards in Toronto where By-law 654-2025 allows a 6-unit houseplex as-of-right — no rezoning, no public hearing, no Council approval required.</li>
  </ul>''',
'''  <p>103 Essa Road confirms a strong development option. This property is a serviced residential lot in the City of Barrie, where up to <strong>four residential units are permitted as-of-right</strong>. Barrie moved ahead of the provincial three-unit minimum, updating its Zoning By-law (2024) to allow up to four units on an eligible residential lot — no rezoning required, subject to the City's site standards.</p>
  <ul>
    <li><strong>The Four-Unit As-of-Right Advantage:</strong> a multiplex of up to four units is permitted on this lot without a rezoning, public hearing, or Council approval — with additional-unit development-charge exemptions and a municipal permit-fee incentive that may further reduce costs (confirmed in Phase 2).</li>
  </ul>'''))

fails = 0
for old, new in R:
    c = s.count(old)
    if c != 1:
        print(f"[FAIL x{c}] {old[:70]!r}")
        fails += 1
    else:
        s = s.replace(old, new)

open(OUT, "w").write(s)

# leftover check — nothing from the source property/city may survive
leftovers = 0
for t in ["Coxwell", "Toronto", "Ward 19", "Beaches", "John Arockiaraj", "johneeraj",
          "647) 223", "654-2025", "474-2023", "Bill 185", "569-2013", "M4L 3B5",
          "6+1", "4+1", "Gerrard", "Danforth", "TTC", "garden suite", "garage",
          "Briarstone"]:
    n = s.count(t)
    if n:
        print(f"LEFTOVER '{t}': {n}")
        leftovers += 1

print(f"done. fails={fails} leftover_tokens={leftovers} -> {OUT}")
